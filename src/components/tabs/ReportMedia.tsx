...
export default function Reports() {
  const [, setFilteredReports] = useState(mockReports);
  ...
  useEffect(() => {
    ...
    const filtered = mockReports.filter((report) => {
      ...
      const isAfterStartDate = startDate
        ...
        : true;
      const isBeforeEndDate = endDate ? reportDate <= new Date(endDate) : true;
      const isWithinDefaultRange =
        !startDate &&
        !endDate &&
        reportDate >= fifteenDaysAgo &&
        reportDate <= today;

      return (isAfterStartDate && isBeforeEndDate) || isWithinDefaultRange;
    });
